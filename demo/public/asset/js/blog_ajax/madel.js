...
links.forEach(function (link) {
     link.addEventListener('click', function (event) {
          ...
          var htmls = '';
          $.ajax({
               url: '/chi-tiet-thu-tuc-hanh-chinh/' + id,
               type: 'GET',
               success: function (response) {
                    if (response.image != null) {
                         ...
                         html += '</div>';
                         html += '<div class="col-md-3" class="col-md-3" style="background: #b6a58d; padding: 15px; border-radius: 8px; height: 100%;">';
                         html += '<h6 style="color: black; font-weight: 700; margin-bottom: 15px;"> Quảng Cáo </h6>';
                         html += '<a class="item" target="_blank" href="' + response.link + '"><img src="' + response.image + '" class="rounded" alt="Cinque Terre" style="width: 100%;"></a>';
                         html += '</div>';
                    } else {

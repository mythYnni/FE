           ...
           <div class="row">
                     ...
                     <div class="widget">
                          ...
                          <div class="categories">
                               <ul>
                                    ...
                                    <li>
                                         <a href="{{route('category_blog_aProce', $value->linkText)}}"><i class="fas fa-folder-open"></i> {{ $value->name }}
                                              @if ($value->PostsProce)
                                              <span>({{$value->PostsProce->count()}})</span>
                                              @else
                                              <span>(0)</span>
                                              @endif
                                         </a>
                ...
                <div class="col-lg-12">
                     <div class="pagination-blog-area">
                          <ul class="pagination">
                               @if (ceil($list_aProce->total() / 8) > 1)
                               <?php
                                 $current_page = isset($_GET['page']) ? $_GET['page'] : '1';
                                 $page = $current_page - 1;
                                 $pages = $current_page + 1;
                                 $maxPage = ceil($list_aProce->total() / 8);
                                 $check = $current_page;
                                 ?>
                               <div>
                                    @if ($check > 1)
                                    ...
                                    @for ($i = 1; $i <= ceil($list_aProce->total() / 8); $i++)
                                         <a class="page-item {{ $i == $list_aProce->currentPage() ? 'active' : '' }}" href="?page={{ $i }}">{{ $i }}</a>
                                         @endfor
